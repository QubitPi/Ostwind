...
class JacksonParserSpec extends JsonDocumentParserSpec {
    ...
    def "Query field cannot be extracted in the case of #description"() {
        ...
        "invalid JSON"  | RuntimeException | IllegalArgumentException | "Ostwind could not process the request because HTTP request body is not properly JSON-formatted: 'invalid JSON'" | "invalid JSON"
        '{"foo": 123}'  | RuntimeException | IllegalArgumentException | "Ostwind could not process the request because payload is missing 'query' field: '{\"foo\": 123}'"               | "JSON not having 'query' field"
    }
}
