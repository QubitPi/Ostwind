 ...
 */
package io.github.qubitpi.ostwind.config;
...
public enum ErrorMessageFormat implements MessageFormatter {
     ...
     */
    EMPTY_LIST("Ostwind could not process the request due to an internal error.", "List cannot be empty"),
    ...
    JSON_DESERIALIZATION_ERROR(
            "Ostwind could not process the request because HTTP request body is not properly JSON-formatted: '%s'",
            "Invalid JSON: '%s'"
    ),
    ...
    INVALID_GRAPHQL_REQUEST(
            "Ostwind could not process the request because %s: '%s'",
